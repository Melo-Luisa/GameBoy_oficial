...
boolean Juiz::hit_esquerda() {
    ...
    return bolinha.getX() - bolinha.getCircleRadius() <= 0 && (bolinha.getY() >= ((barra.move_joy() ))  && bolinha.getY() <= (barra.move_joy() + bar::square_Height));
...
boolean Juiz::hit_superior() {
    return bolinha.getY() - bolinha.getCircleRadius() <= 0;
}

// Função para verificar colisão com a borda inferior
boolean Juiz::hit_inferior() {
    return bolinha.getY() + bolinha.getCircleRadius() >= 300; // Ou outra medida para altura da tela
}
